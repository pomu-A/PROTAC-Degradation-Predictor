...
def get_protac_active_proba(
    ...
    for model_filename in os.listdir(models_dir):
        if model_filename.endswith('.ckpt'):
            ...
            models[model_path] = load_model(model_path).to(device)
...
# when regression
def get_protac_active_regression_proba(
    ...
    # model_to_load = 'best_model' if not use_models_from_cv else 'cv_model'
    # for model_filename in os.listdir(models_dir):
    #     print(f"Checking file: {model_filename}")
    #     if model_to_load not in model_filename:
    #         print(f"  Skipping: {model_to_load} not in filename")
    #         continue
    #     if study_type not in model_filename:
    #         print(f"  Skipping: {study_type} not in filename")
    #         continue
    ...
    for ckpt_path, model in models.items():
        ...
        if not use_xgboost_models:
            ...
            preds[ckpt_path] = pred.detach().cpu().numpy().flatten()
        else:
            X = np.hstack([smiles_emb, poi_emb, e3_emb, cell_emb])
            pred = model.inplace_predict(X)
            preds[ckpt_path] = pred
        print(f"Prediction shape for this model: {pred.shape}")
    # NOTE: The predictions array has shape: (n_models, batch_size)
    preds = np.array(list(preds.values()))
    mean_preds = np.mean(preds, axis=0)
    # Return a single value if not list as input
...
#     # 确保所有输入都是列表
#     if not all(isinstance(i, list) for i in [protac_smiles, e3_ligase, target_uniprot, cell_line]):
#         protac_smiles, e3_ligase, target_uniprot, cell_line = map(lambda x: [x] if not isinstance(x, list) else x, 
#                                                                   [protac_smiles, e3_ligase, target_uniprot, cell_line])
...
#     # 准备嵌入
#     default_protein_emb = np.zeros(next(iter(protein2embedding.values())).shape[0])
#     default_cell_emb = np.zeros(next(iter(cell2embedding.values())).shape[0])
    
#     e3_ligase_uniprot = [config.e3_ligase2uniprot.get(e3, '') for e3 in e3_ligase]
    
#     smiles_emb = np.array([get_fingerprint(s) for s in protac_smiles])
#     cell_emb = np.array([cell2embedding.get(c, default_cell_emb) for c in cell_line])
#     e3_emb = np.array([protein2embedding.get(e3, default_protein_emb) for e3 in e3_ligase_uniprot])
#     poi_emb = np.array([protein2embedding.get(t, default_protein_emb) for t in target_uniprot])
...
#     # 进行预测
#     with torch.no_grad():
#         preds = model(poi_emb, e3_emb, cell_emb, smiles_emb, prescaled_embeddings=False)
#         preds = preds.cpu().numpy()
    
#     # 如果只有一个输入，返回标量；否则返回数组
#     if len(preds) == 1:
#         return {
#             'preds': preds,
#             'mean': preds[0],
